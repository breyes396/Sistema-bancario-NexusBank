    ...
    try {
        ...
        if (!Number.isFinite(parsedPage) || parsedPage < 1) {
            return res.status(400).json({
                success: false,
                message: 'Parametro page invalido'
            });
        }
        ...
        if (!Number.isFinite(parsedLimit) || parsedLimit < 1) {
            return res.status(400).json({
                success: false,
                message: 'Parametro limit invalido'
            });
        }

        let parsedIsActive = true;
        if (typeof isActive !== 'undefined') {
            const normalizedIsActive = String(isActive).toLowerCase();
            if (normalizedIsActive === 'true') {
                ...
                return res.status(400).json({
                    success: false,
                    message: 'Parametro isActive invalido'
                });
            }
        }

        // Crear filtro base - solo productos activos
        const filter = { isActive: parsedIsActive };
        ...
        const options = {
            page: parsedPage,
            limit: parsedLimit,
            sort: { createdAt: -1 } // Ordenar por fecha mas reciente primero
        }
...
export const changeProductStatus = async (req, res) => {
    try {
        ...
        // Determinar si es activacion o desactivacion segun la URL
        const normalizedPath = String(req.path).toLowerCase();
        let isActive;

        if (normalizedPath.endsWith('/deactivate')) {
            isActive = false;
        } else if (normalizedPath.endsWith('/activate')) {
            isActive = true;
        } else {
            return res.status(400).json({
                success: false,
                message: 'Accion de estado invalida'
            });
        }
...
export const getProductsByCategory = async (req, res) => {
    try {
        ...
        const products = await Product.find(filter)
            .limit(parsedLimit)
            .skip((parsedPage - 1) * parsedLimit)
            .sort({ createdAt: -1 }); // Ordenar por mas reciente primero
