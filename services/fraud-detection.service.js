...
export const recordFailedTransaction = async (req, userId, transactionData) => {
  try {
    ...
    if (shouldBlock) {
      ...
      await FailedTransaction.update(
    ...
    const failedCount = await FailedTransaction.count({
      where: {
        userId,
        createdAt: { [Op.gte]: new Date(Date.now() - BLOCKING_THRESHOLDS.timeWindow) },
        isBlocked: false
      }
    });
...
export const detectFraudPatterns = async (req, userId, transactionData) => {
  try {
    ...
    const ipsUsed = await FailedTransaction.findAll({
    ...
    const recentTransactions = await FailedTransaction.count({
      where: {
        userId,
        createdAt: { [Op.gte]: new Date(Date.now() - BLOCKING_THRESHOLDS.rapidTransactionWindow) }
      }
    });
...
export const getUserFailedAttempts = async (userId) => {
  try {
    return await FailedTransaction.findAll({
      where: {
        userId,
        createdAt: { [Op.gte]: new Date(Date.now() - 24 * 60 * 60 * 1000) }
      },
...
export const unblockUser = async (userId, adminId) => {
  try {
    const blockedAttempts = await FailedTransaction.findAll({
      where: {
        userId,
        isBlocked: true,
        blockedUntil: { [Op.gt]: new Date() }
    ...
    for (const attempt of blockedAttempts) {
      attempt.blockedUntil = new Date(); // Ahora
      await attempt.save();
    }

    // Crear registro de alerta resuelta
    await FraudAlert.update(
      { status: 'RESOLVED', reviewedBy: adminId, reviewedAt: new Date() },
      {
        where: {
          userId,
          alertType: 'EXCESSIVE_FAILED_ATTEMPTS',
